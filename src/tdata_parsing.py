...
class QDataStream:
    ...
    def read(self, n=None):
        ...
        if n is not None and len(data) != n:
            raise Exception("неожиданный конец файла")
        return data
...
def decrypt_local(data, key):
    ...
    if encrypted_key != sha1.digest()[:16]:
        raise Exception("не удалось расшифровать")
...
def read_file(name):
    with open(name, "rb") as f:
        magic = f.read(4)
        if magic != b"TDF$":
            raise Exception("неверная магическая последовательность")
...
def read_user_auth(directory, local_key, index=0):
    ...
    if stream.read_uint32() != 0x4B:
        raise Exception("неподдерживаемая конфигурация авторизации")
    ...
    if main_dc not in DC_TABLE:
        raise Exception(f"неподдерживаемый дата-центр: {main_dc}")
    ...
    raise Exception("неверная конфигурация авторизации")
...
async def find_all_key_datas_and_tdata(start_path):
    tdata_paths = []
    for root, _, files in os.walk(start_path):
        norm_root = os.path.normpath(root)
        if "key_datas" in files and os.path.getsize(os.path.join(norm_root, "key_datas")) > 0:
            tdata_paths.append(norm_root)
    return tdata_paths
